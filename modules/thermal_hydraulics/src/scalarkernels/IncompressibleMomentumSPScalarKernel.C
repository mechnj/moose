...
IncompressibleMomentumSPScalarKernelTempl<is_ad>::computeQpResidual()
{
  ...
  for (size_t i = 0; i < _n_segments; ++i)
  {
    ...
    momentum_residual += _rhol * _gravity(_qp, _state) * (*(_lengths[i]))(_qp, _state) *
                         sin((*(_alphas[i]))(_qp, _state));
    // Pump pressure
    momentum_residual -= (*(_dPps[i]))(_qp, _state);
    // Transient term
    momentum_residual +=
        (*(_lengths[i]))(_qp, _state) / (*(_areas[i]))(_qp, _state) * Base::_u_dot[_i];
  }
  // Pressure drop
...
IncompressibleMomentumSPScalarKernelTempl<is_ad>::computeQpJacobian()
{
  if constexpr (!is_ad)
  {
    ...
    for (size_t i = 0; i < _n_segments; ++i)
    {
      ...
      // Friction
      momentum_residual +=
          _fd * (*(_lengths[i]))(_qp, _state) / _Dh * _G / _rhog / (*(_areas[i]))(_qp, _state);
      // Forms
      momentum_residual +=
          (*(_forms_losses[i]))(_qp, _state) * _G / _rhog / (*(_areas[i]))(_qp, _state);
      // Transient term
      momentum_residual +=
          (*(_lengths[i]))(_qp, _state) / (*(_areas[i]))(_qp, _state) * Base::_du_dot_du[_i];
    }
